...
class OauthController extends Controller{
	public function index(){
		$result = self::getAccessTokenAndOpenId($_GET['code']);
        $user = M('game_user');
        if($userinfo = $user->where(array('openid'=>$result['openid']))->find()){
     ...
     */
    public function getCode($state=1, $scope='snsapi_userinfo'){
   
        //公众号的唯一标识
        $appid = 'wxa7aaaa4241ca554e';//八点阳光APPID
        //授权后重定向的回调链接地址，请使用urlencode对链接进行处理
        $redirect_uri = 'http://test.quke8.com/Home/oauth/index';
    ...
    public static function getAccessTokenAndOpenId($code){
    	$appid = 'wxa7aaaa4241ca554e';
    	$appsecret = '73ef07f6de29cba624e8e9776b8c04e1';
        ...
        return $result::callWebServer($url);
    }
    ...
    public static function refreshToken($refreshToken){
        ...
        $queryAction = 'GET';
        $result = new \Org\Util\curl;
        return $result::callWebServer($queryUrl, '', $queryAction);
    }
